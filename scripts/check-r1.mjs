...
for (const page of pages) {
  ...
  if (page.kind !== 'service') {
    ...
    if (!r5Slugs.has(page.slug)) assert(html.includes(escapeHtml(page.lead)), `lead ${page.slug}`);
    if (!r5Slugs.has(page.slug)) for (const item of page.summary ?? page.points) assert(html.includes(escapeHtml(item)), `summary/points ${page.slug}`);
  }
  if (page.slug !== P01) {
