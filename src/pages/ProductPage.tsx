          ...
          <div className="reveal" style={{ ["--reveal-delay" as string]: "100ms" }}>
            ...
            {isSauna && (
              <ul className="mt-6 flex flex-wrap gap-2" aria-label="Входит в цену">
                {standardIncluded.map((s) => <li key={s} className="inline-flex items-center gap-1.5 rounded-full border border-cream-50/12 px-3 py-1 text-xs text-cream-200/85"><CheckIcon className="h-3.5 w-3.5 text-moss-400" /> {s}</li>)}
              </ul>
            )}
        ...
        <div className="mt-16 grid gap-12 lg:grid-cols-[1fr_0.9fr] lg:gap-16">
          <article className="reveal max-w-2xl">
            ...
            <ul className="mt-4 space-y-2.5">
              {p.bullets.map((b) => <li key={b} className="flex gap-3 text-sm sm:text-[15px] leading-relaxed text-cream-200/85"><CheckIcon className="mt-1 shrink-0 text-cedar-400" /> {b}</li>)}
            </ul>
            {p.outro?.map((o) => <p key={o} className="mt-5 text-sm sm:text-[15px] leading-relaxed text-cream-200/80">{o}</p>)}
            ...
            {!p.layout && !p.specs && (
              <div className="reveal rounded-3xl border border-cream-50/8 bg-bark-950/60 p-6 sm:p-8">
                ...
                <ol className="mt-4 space-y-3 text-sm text-cream-200/85">
                  <li className="flex gap-3"><span className="font-display text-cedar-400">01</span> Позвоните или напишите — уточним объём работ и адрес объекта.</li>
                  <li className="flex gap-3"><span className="font-display text-cedar-400">02</span> Мастер или замерщик выезжает на место и считает точную стоимость.</li>
                  <li className="flex gap-3"><span className="font-display text-cedar-400">03</span> Согласуем срок и приступаем к работе.</li>
                </ol>
                <p className="mt-4 text-xs text-cream-300/60">Цена «от {formatPrice(p.price)}» — стартовый ориентир с исходного сайта; итог зависит от объёма и условий на объекте.</p>
                <div className="mt-6 flex flex-wrap gap-3"><LinkButton to={`tel:${company.phoneSecondary.tel}`} variant="ghost" size="sm" onClick={() => track("cta_click", { type: "call2", where: "product", id: p.id })}><PhoneIcon /> {company.phoneSecondary.display} · {company.phoneSecondary.person}</LinkButton></div>
              </div>
            )}
            {p.layout && (
              <div className="reveal rounded-3xl border border-cream-50/8 bg-bark-950/60 p-4 sm:p-6">
                ...
                <ul className="mt-4 divide-y divide-cream-50/8 border-t border-cream-50/8 text-sm">
                  {p.layout.rooms.map((r) => <li key={r.key} className="py-3"><div className="flex justify-between"><span className="font-display text-cream-50">{r.name}</span>{r.size && <span className="text-xs text-cream-300/70">{r.size}</span>}</div><p className="mt-1 text-cream-200/75">{r.items.join(" · ")}</p></li>)}
                </ul>
  ...
  return (
    <div className="bg-bark-900 pt-32 pb-24 min-h-[70vh]">
      <div className="mx-auto max-w-3xl px-4 sm:px-6 text-center">
        ...
        <p className="mt-4 text-cream-200/75">Адрес <code className="rounded bg-cream-50/8 px-1.5 py-0.5 text-sm">{path}</code> не найден. Вот что есть на сайте:</p>
        <ul className="mt-8 grid gap-2 sm:grid-cols-2 text-left">{products.map((p) => <li key={p.id}><Link to={`/product/${p.id}`} className="flex items-center justify-between rounded-2xl border border-cream-50/10 px-4 py-3 text-sm text-cream-100 hover:border-cream-50/30">{p.name} <ArrowIcon className="text-cream-300/60" /></Link></li>)}</ul>
        <div className="mt-8"><LinkButton to="/">На главную</LinkButton></div>
      </div>
    </div>
  );
}
