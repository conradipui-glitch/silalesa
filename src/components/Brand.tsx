  ...
  return <header className={cn("fixed inset-x-0 top-0 z-50 transition-colors duration-300", scrolled || open ? "bg-bark-900/85 backdrop-blur-md border-b border-cream-50/8" : "bg-transparent")}>
    <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex items-center gap-2">
        ...
        <LinkButton to={whatsappUrl("Здравствуйте! Хочу узнать о бане и комплектации. Сайт: https://conradipui-glitch.github.io/silalesa/")} external size="sm" className="hidden sm:inline-flex" onClick={() => track("cta_click", { type: "whatsapp", where: "header" })}>Написать в WhatsApp</LinkButton>
...
export function Footer() {
  return <footer className="border-t border-cream-50/8 bg-bark-950 pb-24 md:pb-10">
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-12 grid gap-10 md:grid-cols-[1.4fr_1fr_1fr]">
      ...
      <div><h3 className="text-xs uppercase tracking-[0.2em] text-cream-300/60">Связаться</h3><ul className="mt-4 space-y-2 text-sm text-cream-200/80"><li><a href={`tel:${company.phonePrimary.tel}`} className="hover:text-cream-50">{company.phonePrimary.display}</a></li><li><a href={`tel:${company.phoneSecondary.tel}`} className="hover:text-cream-50">{company.phoneSecondary.display}</a> <span className="text-cream-300/60">— {company.phoneSecondary.person}</span></li><li className="pt-2 text-cream-300/70">Показ образцов: {company.showroom}</li><li className="text-cream-300/70">Осмотр по предварительной договорённости; время уточните по телефону.</li><li><a href={whatsappUrl("Здравствуйте! Хочу уточнить комплектацию бани. Сайт: https://conradipui-glitch.github.io/silalesa/")} target="_blank" rel="noopener noreferrer" className="text-cedar-300 hover:text-cedar-200">Написать в WhatsApp</a></li></ul></div>
    </div>
